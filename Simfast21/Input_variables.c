...
void get_Simfast21_params(char *basedir){
  ...
  while(fgets(line,length,paramfile)!=NULL){
	...
	if(strcmp(third,"T")==0) global_use_sgrid=1; else global_use_sgrid=0;
      ...
      /* Select Method*/
      else if(strcmp(first,"Instantaneous")==0){
        if(strcmp(third,"T")==0) global_use_Ins=1;
        if(strcmp(third,"F")==0) global_use_Ins=0;
      }
      else if(strcmp(first,"Time_Integrated")==0){
	if(strcmp(third,"T")==0) global_use_Tin=1;
        if(strcmp(third,"F")==0) global_use_Tin=0;
      }

      
      /* Source Model*/
      /* Galaxies*/
      else if(strcmp(first,"Galaxies")==0){
        if(strcmp(third,"T")==0) global_use_gal=1;
        if(strcmp(third,"F")==0) global_use_gal=0;
      }
      else if(strcmp(first,"fesc_GAL")==0)global_fesc_gal=atof(third);
      else if(strcmp(first,"A_GAL")==0)global_A_gal=atof(third);
      else if(strcmp(first,"C_GAL")==0)global_C_gal=atof(third);
      
      /* AGN*/
      else if(strcmp(first,"AGN")==0){
        if(strcmp(third,"T")==0) global_use_agn=1;
        if(strcmp(third,"F")==0) global_use_agn=0;
      }
      else if(strcmp(first,"fesc_AGN")==0)global_fesc_agn=atof(third);
      else if(strcmp(first,"A_AGN")==0)global_A_agn=atof(third);
      else if(strcmp(first,"C_AGN")==0)global_C_agn=atof(third);
      
      
      /* Ionization */
      //      else if(strcmp(first,"fesc")==0)global_fesc_gal=atof(third);
